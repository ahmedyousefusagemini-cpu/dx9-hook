...
// Custom message posted to the render window so the auto-login module can
// trigger the game's Login button handler (FUN_008a8fba) ON THE GAME'S OWN
// MESSAGE THREAD.  HookedWindowProcedure receives it and runs the handler;
// calling the handler directly from the render thread (EndScene/ImGui) crashed
// the client because it does heavy MFC UI work.
// wParam = 0, lParam = pointer to the login dialog (mainwindow + 0x39B948).
#define WM_AUTOLOGIN_SUBMIT (WM_APP + 0x41)
