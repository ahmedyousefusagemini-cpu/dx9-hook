...
| — | `0x005F48B7` | **ndac keystroke feed wrapper** (`__thiscall(editCEnc, dik)`, `RET 4`) — what EVERY real password keystroke runs (`mygameinput` @ `006034b4`): `ndac!#54(dik,1)` via thunk `0x00D02F9C` → IAT `0x01A544AC`; `shift=FUN_005f47ba(editCEnc+0x2C, dik)`; `FUN_005f48e4(dik, shift)` = `ndac!#42(dik,shift)` via `0x00D02F8C` → IAT `0x01A544B0` + `ndac!#85()` via `0x00D02F2C` → IAT `0x01A544C8`. Primes the ndac VM keystroke session that `Ordinal_55` consumes — the auto-login "add a char then delete it" dependency. |
| — | `0x00606E5C` / `0x005F48FA` | killfocus-side ndac feed (`__cdecl`): `FUN_00606e5c(accountCStr)` = `ndac!#34(accountStr)` (`0x00D02F6C` → IAT `0x01A544B8`) + `ndac!#97()` (`0x00D02F4C` → IAT `0x01A544C4`); `FUN_005f48fa(dik, shift)` = `ndac!#41(dik,shift)` (`0x00D02F7C` → IAT `0x01A544B4`) + `ndac!#96()`. Runs inside `Process(..., param_4=1, ...)` — the password-killfocus replay. |
| — | `0x00ED3DCA` | **packet-build ndac pre-feed** — called by `CMsgAccountEx` builder `FUN_00F7F988` (`if (param_5) FUN_00ed3dca(param_5)` = password CEncryptData). Feeds `ndac!#47(handle, nLen)` via `0x0126F28E` → IAT `0x01A54474`, `ndac!#1(0xC8F0B8CC, handle, encLen)` via `0x0126F2AE` → IAT `0x01A5447C`, `ndac!#65(handle, nLen, encLen)` via `0x0126F29E` → IAT `0x01A54478`. `handle = FUN_00500340(this)` (identity) = the CEncryptData object itself. This is the RC5 wire-blob wrap (`CMsgAccountEx+0x114`) input — the VM keystroke-session state fed by `FUN_005F48B7` participates here. |
| — | `0x005F47BA` / `0x0060FFCB` | game shift-check (`__thiscall(sub, dik)`) + key-state reader (`sub+0xC[256]` array snapshot filled by `FUN_005F1830` via the device vtable). |
| — | `0x00ED3602` | edit-sync worker — `GetEncLen(src) → SetEncLen(this) → GetString(src) → SetString(this)` (direction: 0x13BD0 → editCEnc+0x30C). |
...
| `H:\client\ndac.dll` (NOT in `Conquer.exe`) | — | VMProtect-packed network DLL, 18.7 MB, image base `0x10000000`, 139 ordinal-only exports, no names. `Ordinal_8` → RVA `0x00020B60` (dispatcher stub, real impl in encrypted `.rc00`). `Ordinal_55` → RVA `0x00020D80` (same shape). The packet-level encryption key is **hardcoded in the VM bytecode** and is NOT the per-process `CEncryptData` key table at `dlg+0x13BD0`. To recover the algorithm: dump `ndac.dll` from the live process after `Ordinal_8` is first called. |
