...
- `FUN_0117ef25` = the auto-hunt skill rotation processor: iterates the
  configured auto-skill list (`param_1+0x68`; entry `+0x10` = magic sort id),
  per entry does the learned lookup `FUN_011a92b4`, a cooldown check
  `FUN_011a9744`, then fires the current skill via `FUN_011b1ec9` /
  `FUN_011b3503`. (Useful if we ever want overlay-configured hunt skills.)
- `FUN_011354b1` (kill tick) feeds the XP bar via `FUN_011154f5(1)`.
